...
class Downloader(cdk.Stack):
    ...
    def __init__(
        self, scope: Construct, construct_id: str, params=None, **kwargs
    ) -> None:
        ...
        self.ec2_instance_role.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=[
                    f"arn:aws:logs:{region}:{account}:log-group:/aws/PerfBench/downloader:log-stream:"
                ],
            )
        )
        ...
        ssm.StringParameter(
            self,
            "SSM parameter CloudWatch agent configuration for downloader",
            parameter_name="/ONT-performance-benchmark/downloader-cloudwatch-agent-config",
            string_value=open(
                os.path.join(
                    dirname, "assets", "cloudwatch_agent_config_downloader.json"
                )
            ).read(),
        ...
        ssm.StringParameter(
            self,
            "SSM parameter Downloader script",
        ...
        self.downloader_instance = ec2.Instance(
            ...
            machine_image=ec2.MachineImage.generic_linux(
                {
                    "us-west-2": "ami-012bf399e76fe4368",  # Canonical, Ubuntu, 22.04 LTS, arm64 jammy image build on 2024-03-01
                    "me-central-1": "ami-0ae450ff367ed96ed",  # Canonical, Ubuntu, 22.04 LTS, arm64 jammy image build on 2024-11-20
                }
            ),
        ...
        self.downloader_instance.node.add_dependency(
            params.fsx_lustre.cfn_fsx_file_system
        )
        ...
        self.downloader_instance.user_data.add_commands(
            open(
                os.path.join(os.path.dirname(__file__), "assets", "launcher.sh")
            ).read()
        )
        ...
        params.status_parameters.pod5_converter_status.grant_write(
            self.ec2_instance_role
        )
        ssm.StringParameter(
            self,
            "SSM parameter downloader stack name",
