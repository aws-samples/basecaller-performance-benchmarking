...
instance_specs = utils.get_instance_specs(
    "/ONT-performance-benchmark/aws-batch-instance-types"
)
...
def main():
    ...
    if results.empty:
        print(
            "No results found. Make sure to run the benchmark jobs first. Exiting ..."
        )
        return
    ...
    if results.empty:
        print(
            "No results after filtering. Make sure to filter for the tags provided "
    ...
    print(
        'Generating chart "Basecaller runtimes for whole human genome (WHG) at 30x coverage" ...'
    )
...
def filter_results(df: pd.DataFrame):
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "guppy, no modified bases") & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "guppy, modified bases 5mCG") & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "guppy, modified bases 5mCG & 5hmCG")
            & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "dorado, no modified bases") & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
        temp.loc[:, "modified_bases"] = "no modified bases"
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "dorado, modified bases 5mCG")
            & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "dorado, modified bases 5mCG & 5hmCG")
            & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "dorado v0.8.2, no modified bases")
            & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "dorado v0.8.2, modified bases 5mCG")
            & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "dorado v0.8.2, modified bases 5mCG & 5hmCG")
            & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "dorado v0.5.3, no modified bases")
            & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "dorado v0.5.3, modified bases 5mCG")
            & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
    ...
    temp = get_latest_run(
        df[
            (df["tags"] == "dorado v0.5.3, modified bases 5mCG & 5hmCG")
            & (df["status"] == "succeeded")
        ].copy()
    )
    if not temp.empty:
...
def get_latest_run(df: pd.DataFrame):
    ...
    """
    last_runs = pd.merge(
        df.groupby("compute_environment")
        .container_end_time.max()
        .to_frame()
        .reset_index(),
        df,
        left_on=["compute_environment", "container_end_time"],
        right_on=["compute_environment", "container_end_time"],
        how="left",
    )["data_set_id"].to_list()
...
def generate_chart_performance_samples_per_sec(results: pd.DataFrame):
    instance_order = (
        ...
        .sort_values(["samples_per_s"], ascending=False)["display_label"]
        .unique()
    )
    fig = px.bar(
        results[
        ...
        text_auto=".2s",
        orientation="h",
        height=200 + len(instance_order) * 90,
        width=1200,
    )
    ...
    fig.for_each_annotation(
        lambda a: a.update(
            text=a.text.replace("modified_bases=5mCG_5hmCG", "with 5mCG/5hmCG calling")
        )
    )
    fig.for_each_annotation(
        lambda a: a.update(
            text=a.text.replace("modified_bases=5mCG", "with 5mCG calling")
        )
    )
    fig.for_each_annotation(
        lambda a: a.update(
            text=a.text.replace(
                "modified_bases=no modified bases", "without modification calling"
            )
        )
    )
...
def generate_chart_runtime_whg_30x(results: pd.DataFrame):
    instance_order = (
        ...
        .sort_values(["runtime_h"], ascending=True)["display_label"]
        .unique()
    )
    fig = px.bar(
        results[
...
def generate_cost_tables(results: pd.DataFrame):
    ...
    df = pd.concat([temp1, temp2, temp3], ignore_index=True)
    ...
    df_pivot.loc[:, "num_gpus"] = df_pivot.apply(
        lambda row: instance_specs[row.ec2_instance_type.iloc[0]]["GpuInfo"]["Gpus"][0][
            "Count"
        ],
        axis=1,
    )
    df_pivot.loc[:, "cost_per_hour"] = df_pivot.apply(
        lambda row: (
            instance_cost["instances"][row.cost_region.iloc[0]][
                row.ec2_instance_type.iloc[0]
            ]["cost_per_hour"]
            if instance_cost["instances"][row.cost_region.iloc[0]][
                row.ec2_instance_type.iloc[0]
            ]["cost_per_hour"]
            else None
        ),
    ...
    with pd.ExcelWriter(file_name, engine="xlsxwriter") as writer:
        for region in df_pivot["cost_region"].unique():
            ...
            temp.sort_values(
                [("without modification calling", "cost [$]", "per WHG 30x")],
                inplace=True,
            )
            temp.reset_index(inplace=True)
            temp.drop("index", axis=1, level=0, inplace=True)
            temp.index += 1
            temp.loc[:, temp.columns != ("cost_region", "", "")].to_excel(
                writer, sheet_name=region
            )
    return
