...
environment_eu_west_1 = cdk.Environment(
    account=boto3.client("sts").get_caller_identity().get("Account"), region="eu_west_1"
)

# ensure we are in the right region
allowed_regions = ["us-west-2", "me-central-1"]
if environment.region not in allowed_regions:
    raise ValueError(
        f"This CDK application needs to be deployed in one the allowed regions: {allowed_regions}"
...
params.downloader = Downloader(
    app,
    "ONT-PerfBench-Downloader",
    params=params.perfbench.params,
